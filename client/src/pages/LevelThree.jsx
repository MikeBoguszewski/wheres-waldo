...
export default function LevelThree() {
  ...
  useEffect(() => {
    const fetchPositions = async () => {
      try {
        const response = await fetch("https://wheres-waldo-8za6.onrender.com/api/levels/3");
    ...
    const fetchScores = async () => {
      try {
        const response = await fetch("https://wheres-waldo-8za6.onrender.com/api/scores/3");
        const data = await response.json();
        setScores(data);
      } catch (error) {
        console.error("Error fetching data:", error);
    ...
    <>
      ...
      <div className="level">
        <h1>
          ...
          <img src="/assets/odlaw.jpg" className="character"></img>
        </h1>
        <Timer time={time} />
        <div className="gameboard" onClick={handleClick}>
          ...
          {clicked && (
            <div className="popup-box" style={{ top: `${popupCoordinates.y}%`, left: `${popupCoordinates.x}%` }}>
              ...
              <button onClick={() => checkCoordinates("Odlaw")} disabled={foundCharacters.includes("Odlaw")} style={{ backgroundColor: foundCharacters.includes("Odlaw") ? "green" : "" }}>
                <img src="/assets/odlaw.jpg" className="character"></img>
              </button>
            </div>
          )}
