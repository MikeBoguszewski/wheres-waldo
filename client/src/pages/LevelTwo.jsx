...
export default function LevelTwo() {
  ...
  useEffect(() => {
    const fetchPositions = async () => {
      try {
        const response = await fetch("https://wheres-waldo-8za6.onrender.com/api/levels/2");
    ...
    const fetchScores = async () => {
      try {
        const response = await fetch("https://wheres-waldo-8za6.onrender.com/api/scores/2");
        const data = await response.json();
        setScores(data);
      } catch (error) {
        console.error("Error fetching data:", error);
